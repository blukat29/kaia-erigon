...
type KaiaPatriciaContext struct {
	...
	pendingAccounts map[string][]byte // addr[20] => SerialiseV3(acc) or RawBytes
	pendingStorages map[string][]byte // addr[20] || slot[32] => data[32]
	pendingBranches map[string][]byte // prefix[] => data[]
}

func NewKaiaPatriciaContext(step uint64) *KaiaPatriciaContext {
...
func accountToUpdateEth(encAccount []byte) (*Update, error) {
	u := &Update{CodeHash: EmptyCodeHashArray}
	if len(encAccount) == 0 {
...
func (kc *KaiaPatriciaContext) Account(plainKey []byte) (*Update, error) {
	...
	if encAccount, ok := kc.pendingAccounts[string(plainKey)]; ok {
		// TODO-Kaia: accountToUpdateKaia()
		u, err := accountToUpdateEth(encAccount)
		if err != nil {
			return nil, err
		}
